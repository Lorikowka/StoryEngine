...
public class PlayerQuestDataProvider implements ICapabilitySerializable<CompoundTag> {
    ...
    public static final ResourceLocation ID = new ResourceLocation(StoryEngineMod.MOD_ID, "player_quest_data");

    private final PlayerQuestData data = new PlayerQuestData();
    private final LazyOptional<IPlayerQuestData> optional = LazyOptional.of(() -> data);

    @Override
    public <T> LazyOptional<T> getCapability(Capability<T> capability, Direction side) {
        return PlayerQuestDataCapability.QUEST_DATA.orEmpty(capability, optional.cast());
    }
