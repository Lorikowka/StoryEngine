...
> Команды доступны под корнем `/story quest` (старое имя `/quest` — рабочий
> алиас). Справка: `/story help quest`.
...
- изменение title/description/списка задач квеста **прямо во время игры**
  (например, из диалога через `/story tell` + командный блок) с синхронизацией
  всем игрокам онлайн;
...
| Класс | Назначение |
|---|---|
| `quest/story questData.java` | корневая модель квеста (id, title, description, tasks, rewards) |
| `quest/story questTask.java` + подклассы | базовая модель подзадачи и 5 типов (см. ниже) |
| `quest/story questRewards.java`, `quest/ItemReward.java` | награды: команды, опыт, предметы |
| `quest/story questManager.java` | загрузка/сохранение/кэш квестов (Gson) |
| `quest/story questProgressTracker.java` | серверный тик-трекер: считает прогресс LOCATION/ITEM/BLOCK_BREAK/KILL_ENTITY |
...
| `network/story questNetworking.java` | `SimpleChannel`, пакет `S2CSyncQuestDataPacket`, рассылка статусов+данных квестов клиенту |
| `command/story questCommand.java` | вся командная логика `/story quest ...` |
| `client/story questScreen.java` | GUI по клавише `J` |

## Где хранятся квесты

```
config/story_engine/story quests/<id>.json
```
...
- `title`, `description` — можно менять на лету командой `/story quest edit` (см. ниже);
...
- `rewards` — команды/опыт/предметы, выдаются при `/story quest complete`.
...
`ManualQuestTask` автозавершался в первый же тик после `/story quest start`, что
ломало весь квест). Завершить её можно только явно:
`/story quest task complete <player> <questId> <taskId>`, либо форсировать весь
квест сразу через `/story quest complete <player> <id>` (игнорирует список задач).
...
Все команды `/story quest ...` требуют permission level 2 (как `/gamemode`).
`<id>`/`<questId>`/`<taskId>` поддерживают автодополнение по Tab.

### Управление квестами (глобально, не завязано на игрока)

```text
/story quest create <id> [title]                 - создать шаблон JSON
...
`/story quest edit` сохраняет изменение в JSON на диске (переживёт `/story quest reload`
...
/story quest complete <player> <id>              - принудительно COMPLETED, выдаёт rewards
/story quest fail <player> <id>
/story quest reset <player> <id>                 - сбрасывает статус + прогресс подзадач
```
...
Как и `/story quest edit`, все три (`add`/`remove`/`edit`) сохраняют квест на диск
...
/story quest edit find_ancient_key description "Ключ спрятан под старой мельницей, а не в доме."
```
...
```text
/story quest task add location find_ancient_key old_mill minecraft:overworld 120 64 -30 4 Загляните под мельницу
```

## Клиентский GUI (клавиша `J`)

`client/story questScreen.java` — двухпанельный экран:
...
- фон/вкладки/список используют текстуры
  `assets/story_engine/textures/gui/story quest_menu.png` и `quest_widgets.png`
...
2. Для нового *условия* прохождения используйте `/story quest task add`, а не
...
3. `MANUAL`-подзадачи требуют явного завершения — не забывайте вызывать
   `/story quest task complete` (например, из скрипта диалога) или закрывать весь
   квест через `/story quest complete`.
4. После правки JSON руками (не через команды) обязательно `/story quest reload`.
...
4. добавить новую подкоманду `/story quest task add <type> ...` в `QuestCommand.java`,
